...
TokenType Lexer::isKeyword(std::string value) {
  ...
  if (previous == HASHTAG && value == "begin")
    ...
    return IMPORT;
  else if (previous == HASHTAG && value == "module")
    return MODULE;
  else if (value == "null")
    return INULL;
...
void Lexer::parse() {
  while (stream.peek() != EOF) {
    ...
    case '-':
      next == '>'
          ? container.push(Token("->", ARROW, line, column)) //|| stream.get()
      : next == '='
          ? container.push(Token("-=", MINUS_EQ, line, column)) //|| stream.get()
    ...
    case '+':
      next == '='
          ? container.push(Token("+=", PLUS_EQ, line, column)) || stream.get()
    ...
    case '/':
      next == '*'   ? isComment("/*")
      : next == '/' ? container.push(Token("//", DOUBLE_SLASH, line,
                                           column)) //|| stream.get()
                    : container.push(Token("/", SLASH, line, column));
      break;
    case '*':
      next == '='
          ? container.push(Token("*=", STAR_EQ, line, column)) //|| stream.get()
    ...
    case '=':
      next == '=' ? container.push(
                        Token("==", EQUAL_EQ, line, column)) //|| stream.get()
